;; Ported from Truss v2.5.1 (commit 4b16f6b59252873eddb13c05484771a901d523e4):
;; https://github.com/taoensso/truss/blob/v2.5.1/test/taoensso/truss_tests.cljc
;; Jolt-specific branches are documented in ../../README.md.
...
(deftest _submap?
  [(is      (submap? {}       {:a :submap/nx}))
   ...
   (is (not (submap? {}       {:a keyword?})) "Fn preds are nil-punned: called with `(get m k)`")
   (is      (submap? {:a :A}  {:a keyword?}))
   (is      (submap? {}       {:a nil?}))
   (is      (submap? {:a nil} {:a nil?}))
   (is      (submap? {}       {:a any?}))
   (is (not (submap? {}       {:a nil}))     "Data vals aren't nil-punned: they require key presence")
   (is      (submap? {:a nil} {:a nil}))
...
(deftest _error-basics
  (let [ex1 (ex-info "Ex1" {:k1 "v1"})
    ...
    [(is (=       (truss/ex-root          ex2)          ex1))
     ...
     (is
       #?(:jolt
          ;; https://jolt-lang.net/docs/host-interop.html#what-s-shimmed
          ;; Jolt documents .getStackTrace as empty, so Truss has no :trace to
          ;; include in the normalized exception map.
          (let [m (truss/ex-map ex2)]
            (and (submap? m (assoc ex1-map :chain [ex2-map ex1-map]))
                 (not (contains? m :trace))))

          :default
          (submap? (truss/ex-map ex2)
            (assoc ex1-map
              :chain [ex2-map ex1-map]
              :trace #?(:clj #(vector? (force %))
                        :cljs string?)))))

     ;; Message-less errors retain `:type`
     #?@(:jolt
         ;; Jolt 0.8.1 does not retain the cause supplied to
         ;; (Exception. message cause), so use portable ex-info to preserve the
         ;; Truss behavior covered here: a message-less error with a cause.
         ;; https://jolt-lang.net/docs/host-interop.html#what-s-shimmed
         [(is (=       (truss/ex-chain :as-map (ex-info nil {} ex1)) [{:type ex-type} ex1-map]))
          (is (submap? (truss/ex-map           (Exception.))          {:type 'java.lang.Exception}))]

         :clj
         [(is (=       (truss/ex-chain :as-map (Exception. nil ex1)) [{:type 'java.lang.Exception} ex1-map]))
          (is (submap? (truss/ex-map           (Exception.))          {:type 'java.lang.Exception}))])]))
...
(deftest _matching-error
  [(is (truss/error? (truss/matching-error                     (truss/try* ("") (catch :all t t)))))
   ...
   #?(:jolt
      ;; Jolt has no Java proxy support. Keep the direct-error identity behavior
      ;; covered with two distinct exception values.
      ;; https://jolt-lang.net/docs/differences.html#no-jvm-no-java-interop
      (let [kind  (Exception. "Kind")
            error (Exception. "Error")]
        [(is (not (identical? kind error)))
...
(deftest _callsites
  [#?(:jolt
      ;; Observed on Jolt 0.8.1; the online differences guide does not currently
      ;; document this macro-callsite metadata divergence.
      (is (vector? (macro1)) "Jolt retains &form callsite metadata through macro1->2")
      :default
