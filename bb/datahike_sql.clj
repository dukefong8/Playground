...
(defn like
  [s pattern]
  (boolean (re-matches (like-pattern->regex pattern) (str s))))
...
(defn schema-tx
  ...
  (mapv (fn [[ident attr]]
          (assoc attr
                 :db/ident ident
                 :db/cardinality (:db/cardinality attr :db.cardinality/one)))
        schema))

(defn stable-db-id
  [path]
  (java.util.UUID/nameUUIDFromBytes
   (.getBytes (.getAbsolutePath (io/file path)) "UTF-8")))
...
(defn get-conn
  []
  (d/connect (ensure-database!)))

;; loading the data

;; eid base
(def data-eid-base        1000000000)
(def aka-name-base        (+ data-eid-base 1000000))
(def aka-title-base       (+ data-eid-base 2000000))
(def cast-info-base       (+ data-eid-base 100000000))
(def char-name-base       (+ data-eid-base 10000000))
(def comp-cast-type-base  data-eid-base)
(def company-name-base    (+ data-eid-base 3000000))
(def company-type-base    (+ data-eid-base 10))
(def complete-cast-base   (+ data-eid-base 4000000))
(def info-type-base       (+ data-eid-base 1000))
(def keyword-base         (+ data-eid-base 5000000))
(def kind-type-base       (+ data-eid-base 20))
(def link-type-base       (+ data-eid-base 30))
(def movie-companies-base (+ data-eid-base 20000000))
(def movie-info-base      (+ data-eid-base 30000000))
(def movie-info-idx-base  (+ data-eid-base 40000000))
(def movie-keyword-base   (+ data-eid-base 50000000))
(def movie-link-base      (+ data-eid-base 100000))
(def name-base            (+ data-eid-base 60000000))
(def person-info-base     (+ data-eid-base 70000000))
(def role-type-base       (+ data-eid-base 50))
(def title-base           (+ data-eid-base 80000000))

(defn- add-comp-cast-type [reader]
  (map (fn [[id content]]
         (datom (+ ^long comp-cast-type-base (Long/parseLong id))
                :comp-cast-type/kind content))
       (read-csv reader)))

(defn- add-company-type [reader]
  (map (fn [[id content]]
         (datom (+ ^long company-type-base (Long/parseLong id))
                :company-type/kind content))
       (read-csv reader)))

(defn- add-kind-type [reader]
  (map (fn [[id content]]
         (datom (+ ^long kind-type-base (Long/parseLong id))
                :kind-type/kind content))
       (read-csv reader)))

(defn- add-link-type [reader]
  (map (fn [[id content]]
         (datom (+ ^long link-type-base (Long/parseLong id))
                :link-type/link content))
       (read-csv reader)))

(defn- add-role-type [reader]
  (map (fn [[id content]]
         (datom (+ ^long role-type-base (Long/parseLong id))
                :role-type/role content))
       (read-csv reader)))

(defn- add-info-type [reader]
  (map (fn [[id content]]
         (datom (+ ^long info-type-base (Long/parseLong id))
    ...
    (comp
      (map (fn [[id movie linked-movie link-type]]
             (let [eid (+ ^long movie-link-base (Long/parseLong id))]
               [(datom eid :movie-link/movie
                         (+ ^long title-base (Long/parseLong movie)))
                (datom eid :movie-link/linked-movie
                         (+ ^long title-base (Long/parseLong linked-movie)))
                (datom eid :movie-link/link-type
      ...
      (map
        (fn [[id person name imdb-index name-pcode-cf
          ...
          (let [eid (+ ^long aka-name-base (Long/parseLong id))]
            (cond-> [(datom eid :aka-name/person
                              (+ ^long name-base (Long/parseLong person)))
                     (datom eid :aka-name/name name)]
              (not (s/blank? imdb-index))
              (conj (datom eid :aka-name/imdb-index imdb-index))
              (not (s/blank? name-pcode-cf))
              (conj (datom eid :aka-name/name-pcode-cf name-pcode-cf))
              (not (s/blank? name-pcode-nf))
              (conj (datom eid :aka-name/name-pcode-nf name-pcode-nf))
              (not (s/blank? surname-pcode))
              (conj (datom eid :aka-name/surname-pcode surname-pcode))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id movie title imdb-index kind production-year phonetic-code
          ...
          (let [eid (+ ^long aka-title-base (Long/parseLong id))]
            (cond-> [(datom eid :aka-title/movie
                              (+ ^long title-base (Long/parseLong movie)))
                     (datom eid :aka-title/title title)]
              (not (s/blank? imdb-index))
              (conj (datom eid :aka-title/imdb-index imdb-index))
              (not (s/blank? kind))
              (conj (datom eid :aka-title/kind
                             (+ ^long kind-type-base (Long/parseLong kind))))
              (not (s/blank? production-year))
              (conj (datom eid :aka-title/production-year
                             (Long/parseLong production-year)))
              (not (s/blank? phonetic-code))
              (conj (datom eid :aka-title/phonetic-code phonetic-code))
              (not (s/blank? episode-of))
              (conj (datom eid :aka-title/episode-of
                             (+ ^long title-base (Long/parseLong episode-of))))
              (not (s/blank? season-nr))
              (conj (datom eid :aka-title/season-nr (Long/parseLong season-nr)))
              (not (s/blank? episode-nr))
              (conj (datom eid :aka-title/episode-nr (Long/parseLong episode-nr)))
              (not (s/blank? note))
              (conj (datom eid :aka-title/note note))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id name country-code imdb-id name-pcode-nf name-pcode-sf]]
          (let [eid (+ ^long company-name-base (Long/parseLong id))]
            (cond-> [(datom eid :company-name/name name)]
              (not (s/blank? country-code))
              (conj (datom eid :company-name/country-code country-code))
              (not (s/blank? imdb-id))
              (conj (datom eid :company-name/imdb-id (Long/parseLong imdb-id)))
              (not (s/blank? name-pcode-nf))
              (conj (datom eid :company-name/name-pcode-nf name-pcode-nf))
              (not (s/blank? name-pcode-sf))
              (conj (datom eid :company-name/name-pcode-sf name-pcode-sf))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id movie subject status]]
          (let [eid (+ ^long complete-cast-base (Long/parseLong id))]
            [(datom eid :complete-cast/movie
                      (+ ^long title-base (Long/parseLong movie)))
             (datom eid :complete-cast/subject
                      (+ ^long comp-cast-type-base (Long/parseLong subject)))
             (datom eid :complete-cast/status
      ...
      (map
        (fn [[id keyword phonetic-code]]
          (let [eid (+ ^long keyword-base (Long/parseLong id))]
            [(datom eid :keyword/keyword keyword)
             (datom eid :keyword/phonetic-code phonetic-code)])))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id name imdb-index imdb-id name-pcode-nf surname-pcode]]
          (let [eid (+ ^long char-name-base (Long/parseLong id))]
            (cond-> [(datom eid :char-name/name name)]
              (not (s/blank? imdb-index))
              (conj (datom eid :char-name/imdb-index imdb-index))
              (not (s/blank? imdb-id))
              (conj (datom eid :char-name/imdb-id (Long/parseLong imdb-id)))
              (not (s/blank? name-pcode-nf))
              (conj (datom eid :char-name/name-pcode-nf name-pcode-nf))
              (not (s/blank? surname-pcode))
              (conj (datom eid :char-name/surname-pcode surname-pcode))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id movie company company-type note]]
          (let [eid (+ ^long movie-companies-base (Long/parseLong id))]
            (cond-> [(datom eid :movie-companies/movie
                              (+ ^long title-base (Long/parseLong movie)))
                     (datom eid :movie-companies/company
                              (+ ^long company-name-base
                                 (Long/parseLong company)))
                     (datom eid :movie-companies/company-type
                              (+ ^long company-type-base
                                 (Long/parseLong company-type)))]
              (not (s/blank? note))
              (conj (datom eid :movie-companies/note note))))))
      cat)
    (read-csv reader)))
    ...
    (comp
      (map (fn [[id movie info-type info note]]
             (let [eid (+ ^long movie-info-base (Long/parseLong id))]
               (cond-> [(datom eid :movie-info/movie
                                 (+ ^long title-base (Long/parseLong movie)))
                        (datom eid :movie-info/info-type
                                 (+ ^long info-type-base
                                    (Long/parseLong info-type)))
                        (datom eid :movie-info/info info)]
                 (not (s/blank? note))
                 (conj (datom eid :movie-info/note note))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id movie info-type info note]]
          (let [eid (+ ^long movie-info-idx-base (Long/parseLong id))]
            (cond-> [(datom eid :movie-info-idx/movie
                              (+ ^long title-base (Long/parseLong movie)))
                     (datom eid :movie-info-idx/info-type
                              (+ ^long info-type-base (Long/parseLong info-type)))
                     (datom eid :movie-info-idx/info info)]
              (not (s/blank? note))
              (conj (datom eid :movie-info-idx/note note))))))
      cat)
    (read-csv reader)))
    ...
    (comp
      (map (fn [[id movie keyword]]
             (let [eid (+ ^long movie-keyword-base (Long/parseLong id))]
               [(datom eid :movie-keyword/movie
                         (+ ^long title-base (Long/parseLong movie)))
                (datom eid :movie-keyword/keyword
      ...
      (map
        (fn [[id name imdb-index imdb-id gender name-pcode-cf name-pcode-nf
          ...
          (let [eid (+ ^long name-base (Long/parseLong id))]
            (cond-> [(datom eid :name/name name)]
              (not (s/blank? imdb-index))
              (conj (datom eid :name/imdb-index imdb-index))
              (not (s/blank? imdb-id))
              (conj (datom eid :name/imdb-id (Long/parseLong imdb-id)))
              (not (s/blank? gender))
              (conj (datom eid :name/gender gender))
              (not (s/blank? name-pcode-cf))
              (conj (datom eid :name/name-pcode-cf name-pcode-cf))
              (not (s/blank? name-pcode-nf))
              (conj (datom eid :name/name-pcode-nf name-pcode-nf))
              (not (s/blank? surname-pcode))
              (conj (datom eid :name/surname-pcode surname-pcode))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id person info-type info note]]
          (let [eid (+ ^long person-info-base (Long/parseLong id))]
            (cond-> [(datom eid :person-info/person
                              (+ ^long name-base (Long/parseLong person)))
                     (datom eid :person-info/info-type
                              (+ ^long info-type-base (Long/parseLong info-type)))
                     (datom eid :person-info/info info)]
              (not (s/blank? note))
              (conj (datom eid :person-info/note note))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id title imdb-index kind production-year imdb-id phonetic-code
          ...
          (let [eid (+ ^long title-base (Long/parseLong id))]
            (cond-> [(datom eid :title/title title)
                     (datom eid :title/kind
                              (+ ^long kind-type-base (Long/parseLong kind)))]
              (not (s/blank? imdb-index))
              (conj (datom eid :title/imdb-index imdb-index))
              (not (s/blank? production-year))
              (conj (datom eid :title/production-year
                             (Long/parseLong production-year)))
              (not (s/blank? imdb-id))
              (conj (datom eid :title/imdb-id (Long/parseLong imdb-id)))
              (not (s/blank? phonetic-code))
              (conj (datom eid :title/phonetic-code phonetic-code))
              (not (s/blank? episode-of))
              (conj (datom eid :title/episode-of
                             (+ ^long title-base (Long/parseLong episode-of))))
              (not (s/blank? season-nr))
              (conj (datom eid :title/season-nr (Long/parseLong season-nr)))
              (not (s/blank? episode-nr))
              (conj (datom eid :title/episode-nr
                             (Long/parseLong episode-nr)))
              (not (s/blank? series-years))
              (conj (datom eid :title/series-years series-years))))))
      cat)
    (read-csv reader)))
      ...
      (map
        (fn [[id person movie person-role note nr-order role]]
          (let [eid (+ ^long cast-info-base (Long/parseLong id))]
            (cond-> [(datom eid :cast-info/person
                              (+ ^long name-base (Long/parseLong person)))
                     (datom eid :cast-info/movie
                              (+ ^long title-base (Long/parseLong movie)))
                     (datom eid :cast-info/role
                              (+ ^long role-type-base (Long/parseLong role)))]
              (not (s/blank? person-role))
              (conj (datom eid :cast-info/person-role
                             (+ ^long char-name-base (Long/parseLong person-role))))
              (not (s/blank? note))
              (conj (datom eid :cast-info/note note))
              (not (s/blank? nr-order))
              (conj (datom eid :cast-info/nr-order
...
(defn- load-table
  "Open a CSV reader, transact rows in chunks, close reader, show timing."
  [conn start label add-fn csv-file]
  (with-open [rdr (io/reader csv-file)]
    (doseq [tx-data (partition-all 10000 (add-fn rdr))]
      (when (seq tx-data)
...
(defn db []
  ...
  (let [cfg        (db-config)
        ...
        conn       (d/connect (ensure-database!))
...
(def queries (into []
                   ...
                   (sort (keys (ns-publics 'datahike-sql)))))

(def result-filename "datahike_onepass_time.csv")

(defn -main [&opts]
  (println "The Join Order Benchmark 1Pass test ...")

  (with-open [w (io/writer result-filename)]
    (csv/write-csv w [["Query Name" "Execution Time (ms)" "Result Count"]])
    (doseq [q queries]
      (let [qname     (s/replace (name q) "q-" "")
            _         (print (str "  " qname "... "))
            query     (-> q (#(ns-resolve 'datahike-sql %)) var-get)
            started   (System/currentTimeMillis)
            result    (d/q query (d/db @conn))
            exec-time (- (System/currentTimeMillis) started)]
        (println (str exec-time " ms"))
        (csv/write-csv w [[qname exec-time (count result)]]))))
  (d/release @conn)
